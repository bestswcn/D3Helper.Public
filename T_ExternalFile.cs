                ...
                public static void Load()
                {
                    try
                    {
                        ...
                        for (int i = 0; i < AllLines.Count(); i++)
                        {
                            ...
                            if (line1.StartsWith("*"))
                            {
                                ConditionType type1 = (ConditionType) Enum.Parse(typeof (ConditionType), line1.TrimStart('*'));

                                string tooltip = AllLines[i + 1].TrimStart('-', ' ');

                                A_Collection.Presets.Manual.Tooltips.ConditionTypes.Add(type1, tooltip);
                            }
